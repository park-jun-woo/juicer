---
name: codistill
description: Static analysis tool that extracts OpenAPI specs, DDL schemas, and SQL skeletons from web framework projects (Go+Gin, NestJS, FastAPI, Spring Boot). Use when you need to generate or update API specs, extract endpoint metadata, manage DDL migrations, scaffold sqlc queries, or merge scan results with existing OpenAPI specs.
license: MIT
metadata:
  author: park-jun-woo
  version: "0.1.2"
...
- Extract OpenAPI 3.0 specs from Go+Gin, NestJS, FastAPI, or Spring Boot projects
...
- The target framework is not Go+Gin, NestJS, FastAPI, or Spring Boot
...
| `codist scan --framework <fw> [project-root]` | Override framework detection (gogin, nestjs, fastapi, spring) |
...
Framework is auto-detected. Override with `--framework gogin|nestjs|fastapi|spring`.
...
- **Multi-framework** — Go+Gin (`go/ast` + `go/types`), NestJS (tree-sitter TypeScript), FastAPI (tree-sitter Python), Spring Boot (tree-sitter Java). Auto-detected from project files.
...
| `no endpoints found` | No framework router detected | Ensure project uses Gin/NestJS/FastAPI/Spring Boot |
| `unknown framework` | Auto-detection failed | Use `--framework gogin\|nestjs\|fastapi\|spring` |
