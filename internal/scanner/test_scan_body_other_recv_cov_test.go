...
//ff:what TestScanBody_OtherRecvCov 테스트
package scanner
...
func TestScanBody_OtherRecvCov(t *testing.T) {
	ep := &Endpoint{}
	idx := &funcIndex{byPos: make(map[token.Pos]*ast.FuncDecl), info: make(map[token.Pos]*types.Info)}
	call := &ast.CallExpr{Fun: &ast.SelectorExpr{X: &ast.Ident{Name: "other"}, Sel: &ast.Ident{Name: "Do"}}}
