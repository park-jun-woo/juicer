...
type scanContext struct {
	...
	basePaths map[string]string            // prefixKey(file, varName) -> basePath
	groups    []routeGroup                 // all route groups across files
	schemas   map[string]*sitter.Node      // schemaVarName -> z.object() node
	prefixMap map[string]string            // prefixKey(file, varName) -> resolved prefix
	imports   map[string]map[string]string // file -> imported ident -> definition file path
	absRoot   string
}
