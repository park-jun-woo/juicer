...
func collectDTORequests(ep endpointInfo, imports map[string]string, absFile string, epIdx int) []dtoRequest {
	var reqs []dtoRequest
	if ep.bodyType != "" {
		reqs = append(reqs, dtoRequest{
			typeName: ep.bodyType, imports: imports,
			referrer: absFile, epIdx: epIdx, isBody: true,
		})
	}
	if needsResponseDTO(ep.returnType) {
		reqs = append(reqs, dtoRequest{
			typeName: ep.returnType, imports: imports,
			referrer: absFile, epIdx: epIdx, isBody: false,
		})
	}
	return reqs
}
