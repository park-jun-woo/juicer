...
//ff:what TestScanBody_CtxFormFile 테스트
package gogin
...
func TestScanBody_CtxFormFile(t *testing.T) {
	body := &ast.BlockStmt{
		List: []ast.Stmt{
			&ast.ExprStmt{
				X: &ast.CallExpr{
	...
	if len(ep.Request.Files) != 1 {
		t.Errorf("expected 1 file, got %d", len(ep.Request.Files))
	}
}
