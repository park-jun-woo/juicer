...
//ff:what TestResolveNestedFields_Slice 테스트
package gogin
...
func TestResolveNestedFields_Slice(t *testing.T) {
	...
	sl := types.NewSlice(innerNamed)

	visited := make(map[string]bool)
	fields := resolveNestedFields(sl, visited)
	if len(fields) != 1 {
