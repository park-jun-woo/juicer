...
//ff:what 함수 body에서 echo.Context 메서드 호출을 탐색하여 요청/응답 정보를 수집한다
package echo
...
func scanBody(ep *scanner.Endpoint, body *ast.BlockStmt, ctxName string, info *types.Info, idx *funcIndex, source string) {
	...
	ast.Inspect(body, func(n ast.Node) bool {
		...
		if !ok {
			// echo.Context가 아닌 함수 호출이면 1-depth 추적 확인
			if source == "handler" {
		...
		if !ok || recvIdent.Name != ctxName {
			// echo.Context가 아닌 메서드 호출이면 1-depth 추적 확인
			if source == "handler" {
				checkOneDepthCall(ep, call, ctxName, info, idx)
			}
			return true
		...
		if bindMethods[methodName] {
			...
			handleQueryParam(ep, call, methodName)
