...
//ff:what 핸들러 body의 함수 호출에서 echo.Context를 인자로 넘기면 해당 함수 body를 1-depth 추적하고, caller 인자에서 상태코드와 응답 타입을 전파한다
package echo
...
func checkOneDepthCall(ep *scanner.Endpoint, call *ast.CallExpr, ctxName string, info *types.Info, idx *funcIndex) {
	if info == nil {
		return
	}
	...
	targetCtxName := echoCtxParamNameInfo(fnDecl.Type, fnInfo)
	if targetCtxName == "" {
		return
	}
