...
func rescanCalleeWithPrefixDepth(call *ast.CallExpr, argIdx int, prefix string, parent *routerInfo, ctx *groupArgCtx, depth int) {
	...
	paramName := fiberRouterParamAtIndex(fnDecl, fnInfo, argIdx)
	if paramName == "" {
		return
	}

	targetFile := resolveTargetFilePath(targetPos, ctx)
	targetFiberAlias := resolveTargetFiberAlias(targetPos, ctx)
	if targetFiberAlias == "" {
		targetFiberAlias = ctx.fiberAlias
	}
	...
	walkStmts(fnDecl.Body.List, targetFiberAlias, targetFile, ctx.fset, targetRouters, &eps, localMap)
